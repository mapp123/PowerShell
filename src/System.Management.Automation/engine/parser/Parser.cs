        ...
        private StatementAst UsingStatementRule(Token usingToken)
        {
            ...
            if (moduleImportException == null && isConstant && !usesWildcard && moduleInfos != null && moduleInfos.Any())
            {
                ...
                if (module.Path.EndsWith(".dll", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    try
                    {
                        AssemblyName.GetAssemblyName(module.Path);
                    }
                    ...
                    var keywordReader = new DSLKeywordMetadataReader(module);
                    IEnumerable<DynamicKeyword> keywords = keywordReader.ReadDslKeywords(out keywordErrors);
